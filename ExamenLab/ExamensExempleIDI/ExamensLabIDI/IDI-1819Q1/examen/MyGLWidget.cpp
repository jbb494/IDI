...
void MyGLWidget::iniEscena ()
{
	...
	radiEsc = sqrt(20*20 + 4*4 + 12*12)/2.f;
...
void MyGLWidget::resizeGL (int w, int h)
{
	...
	if(ra < 1)
	{
		...
		FOV2 = 2.f*glm::atan(glm::tan(M_PI/2.f )/ra);
...
void MyGLWidget::modelTransformModel2 ()
{
	...
	TG = glm::translate(TG, glm::vec3(-7.f+offsetPorter[0],0.f+offsetPorter[1],0.f+offsetPorter[2]) );
	TG  = glm::rotate(TG, (float)M_PI/2, glm::vec3(0, 1, 0));
	TG = glm::scale(TG, glm::vec3(4*escalaModel,4* escalaModel,4* escalaModel));
	TG = glm::translate(TG, -centreBaseModel );
...
void MyGLWidget::projectTransform2 ()
{
	...
	Proj = glm::perspective(FOV2, ra, radiEsc, 3.5f*radiEsc);
...
{
	glm::mat4 Proj;  // Matriu de projecció
	Proj = glm::perspective(FOV, ra, 0.01f, 3.5f*radiEsc);
...
{
	glm::mat4 View;  // Matriu de posició i orientació
	View = glm::lookAt(glm::vec3(8,5,0),glm::vec3(-7,0,0), glm::vec3(0,1,0));
	glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &View[0][0]);
...
bool MyGLWidget::aturaPorter()
{
	...
	atura = (posPilota[2] > offsetPorter[2] -1.f && posPilota[2] < offsetPorter[2] +1.f) && posPilota[0] < -5.8f && posPilota[0] > -6.3;
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
	...
	switch (event->key()) {
		...
		case Qt::Key_Left: { // reinicia posició pilota
					    if (offsetPorter[2] < 1) offsetPorter = glm::vec3(offsetPorter[0],offsetPorter[1], offsetPorter[2]+0.1 );			
					    modelTransformModel2();	
					    break;
				    }
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
	...
	if (DoingInteractive == ROTATE && cameraGlob == false)
	{
		// Fem la rotació
...
void MyGLWidget::canviaCamera(bool a)
{
	makeCurrent();
	if (!a)
	{
...
void MyGLWidget::canviaGraus(int graus)
{
	anglePilota = -graus;	
}
void MyGLWidget::reset()
{
	if(cameraGlob)
	{
		canviaCamera(!cameraGlob);
		emit canviaCameraSignal();
	}	
	Psi = M_PI/4;
	Theta  = 0;
	viewTransform();
