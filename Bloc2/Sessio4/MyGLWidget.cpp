...
void MyGLWidget::initializeGL ()
{
  ...
  transf_vec = std::vector<transformacio> (N_instancies);
...
void MyGLWidget::tracta_valueChanged(int a) //slot
{
	makeCurrent();
	cam.zoom = a;
	projectTransform();
...
void MyGLWidget::canvia_Personatges(bool a) //slot
{
	makeCurrent();
	...
	for(int i = 0; i<N_instancies; i++)
	{
		if(a == true)
		{
			Instancies[i].vao_general = (VAO*)&VAOs[1];	 //LEGOMAN
...
void MyGLWidget::canvia_Prespectiva(bool a) //Slot
{
	makeCurrent();

	if(a == true)opt = ORTOGONAL;
	else opt = PRESPECTIVA;
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R:{
		for(int i = 0; i<N_instancies; i++)
		{
	  Instancies[i].transf.rotacio += glm::vec3(0.f,M_PI/12,0.f);
		}
	  break;
	} case Qt::Key_O:{
...
void MyGLWidget::projectTransform()
{
	...
	{
		if(cam.ra>1 )Proj = glm::ortho(-1*esf.r*cam.ra/cam.zoom,esf.r*cam.ra/cam.zoom,-1*esf.r/cam.zoom,esf.r/cam.zoom, cam.znear, cam.zfar);
		else Proj = glm::ortho(-1*esf.r/cam.zoom,esf.r/cam.zoom,-1*esf.r/cam.ra/cam.zoom,esf.r/cam.ra/cam.zoom, cam.znear, cam.zfar);
	}
